...
Looking for Form 8609 via an exact Product Number match:
![results](md_img/inconsistent_results3.png)

*Should I email them?* ;)

### Output for U2
The script outputs folders with each folder name being representative of the exact IRS.g*v's "publication number"/"form number" for each tax doc. The names of the files should be an exact match as well of the form number and the year of that form revision.
...
* Finally used a dictionary to keep track of items outside of it and having the dict itself be independent of pending JSON transformation, but instead used for a different type of tracking (the 'occurrences' variable in utility1.py)
